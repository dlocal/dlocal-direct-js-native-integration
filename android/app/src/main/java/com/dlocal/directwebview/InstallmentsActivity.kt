...
class InstallmentsActivity : AppCompatActivity() {
    ...
    private fun setupToolbar() {
        ...
        supportActionBar?.title = "Get Installments Plan"
                    ...
                    try {
                        ...
                        when (type) {
                            "success" -> {
                                ...
                                Toast.makeText(this@InstallmentsActivity, "Installments plan retrieved successfully!", Toast.LENGTH_SHORT).show()
                            }
                            "error" -> {
                                tvResponse.text = "ERROR:\n${payload.toString(2)}"
                                Toast.makeText(this@InstallmentsActivity, "Error: ${payload.optString("message", "Unknown error")}", Toast.LENGTH_LONG).show()
                            }
                        }
                    } catch (e: Exception) {
                        tvResponse.text = "ERROR parsing response:\n$message"
                        Toast.makeText(this@InstallmentsActivity, "Error parsing response", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }, "AndroidBridge")
    ...
    private fun onGetInstallmentsClick() {
        ...
        val amountText = findViewById<TextInputEditText>(R.id.etAmount).text.toString()
        val currency = findViewById<TextInputEditText>(R.id.etCurrency).text.toString()
        val country = findViewById<TextInputEditText>(R.id.etCountry).text.toString()
        val bin = findViewById<TextInputEditText>(R.id.etBin).text.toString()

        // Validate inputs
        if (publicKey.isBlank() || amountText.isBlank() || currency.isBlank() || 
            country.isBlank() || bin.isBlank()) {
        ...
        sendGetInstallmentsPlan(publicKey, amount, currency, country, bin)
    ...
    private fun sendGetInstallmentsPlan(publicKey: String, amount: Int, currency: String, country: String, bin: String) {
        ...
        val args = JSONArray().put(amount).put(currency).put(country).put(bin)
